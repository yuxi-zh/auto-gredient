...
public:
  Input<Buffer<float>> guide{"guide", 4};
  Input<Buffer<float>> grid{"grid", 5};
  Input<Buffer<float>> coeff{"coeff", 4};
  Output<Buffer<float>> grad_guide{"grad_guide", 4};
  Output<Buffer<float>> grad_grid{"grad_grid", 5};
  ...
  void generate() {
    ...
    Func d_guide = grad_guide;
    Func d_grid = grad_grid;
